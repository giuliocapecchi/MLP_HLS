...
\paragraph{Implementation}
In this section, we analyze the Place and Route stage of the ConvNet design implementation. This stage provides a detailed report on the physical implementation of the design on the FPGA. 
...
\subsection{Vivado RTL Synthesis}
...
\begin{table}[H]
    \label{tab:resource-utilization-guideline}
...
The synthesis results indicate that the design fits comfortably within the available resources of the target FPGA. The DSP utilization is relatively high (83.33\%), which might require attention during optimization. The achieved clock period (3.232 ns) is well below the target period (10.000 ns), ensuring sufficient timing slack at this stage.

\subsection{Vivado Place \& Route}
...
The results of the place-and-route step are summarized in Table~\ref{tab:place-route-results}:
...
\textbf{Analysis:} The resource utilization remains within acceptable limits after place-and-route, with slight changes compared to the synthesis stage. The timing was successfully met, with a post-route clock period of 9.354 ns, which is below the target clock period. This indicates that the design is capable of operating at the desired frequency after routing delays are considered.

\textbf{Conclusion:} The implementation phase results confirm that the design fits within the available resources and meets timing requirements. The high DSP utilization (83.33\%) across both stages suggests potential optimization opportunities in future iterations.
